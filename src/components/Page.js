...
const Page = ({ 
  ...
  const handleRotateImage = (e, row, col) => {
    ...
    if (imageUrl) {
      // Resmi çevir ve çevrilmiş halini kaydet
      rotateImage(imageUrl, (rotatedImageDataUrl) => {
        // Çevrilmiş resme de rounded corners uygula
        applyRoundedCorners(rotatedImageDataUrl, (finalImageDataUrl) => {
          // Sadece bu cep için güncelleme yap
          updatePageWithState(
            (prevContent) => ({ ...prevContent, [key]: finalImageDataUrl }),
            undefined,
            (prevRotatedImages) => ({ ...prevRotatedImages, [key]: true }) // Çevrilmiş olarak işaretle
          );
        });
      });
    }
    ...
    if (imageUrl) {
      ...
      rotateImage(imageUrl, (rotatedImageDataUrl) => {
        ...
        applyRoundedCorners(rotatedImageDataUrl, (finalImageDataUrl) => {
          ...
          const backKey = `back-${cellKey}`;
          updatePageWithState(
            undefined,
            (prevBackContent) => ({ ...prevBackContent, [cellKey]: finalImageDataUrl }),
            (prevRotatedImages) => ({ ...prevRotatedImages, [backKey]: true }) // Arka yüz için çevrilmiş olarak işaretle
          );
        });
      });
    }
